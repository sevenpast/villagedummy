...
import { IntelligentDocumentAnalyzer } from '../../../../services/intelligent-document-analyzer';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📤 Starting intelligent upload for:', file.name);

    // Step 1: Intelligent Document Analysis
    console.log('🧠 Step 1: Intelligent document analysis');
    const analyzer = new IntelligentDocumentAnalyzer();
    const analysisResult = await analyzer.analyzeDocument(file.name);
    ...
    console.error('❌ Intelligent upload failed:', error);
    return NextResponse.json({
      error: 'Upload failed',
