-- ============================================================================
-- CORRECTLY FIX DUPLICATE get_user_tasks FUNCTIONS
-- ============================================================================

-- First, let's see what functions exist with their exact signatures
SELECT 
  'Current get_user_tasks functions:' as info,
  ...
  oid
FROM pg_proc 
WHERE proname = 'get_user_tasks'
AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public');

-- Drop functions by specifying exact argument lists
-- This is the correct way to handle ambiguous function names
DROP FUNCTION IF EXISTS public.get_user_tasks(UUID);
DROP FUNCTION IF EXISTS public.get_user_tasks(TEXT);
DROP FUNCTION IF EXISTS public.get_user_tasks(UUID, INTEGER);
DROP FUNCTION IF EXISTS public.get_user_tasks();

-- Also try dropping by OID if the above doesn't work
DO $$
DECLARE
    func_record RECORD;
BEGIN
    FOR func_record IN 
        SELECT oid, proname, proargtypes::regtype[] as arg_types
        FROM pg_proc 
        WHERE proname = 'get_user_tasks' 
        AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
    LOOP
        BEGIN
            EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.oid::regproc;
            RAISE NOTICE 'Dropped function: % with args: %', func_record.proname, func_record.arg_types;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'Could not drop function: % - Error: %', func_record.oid::regproc, SQLERRM;
        END;
    END LOOP;
END $$;
...
SELECT 
  'Functions after dropping:' as info,
  proname as function_name,
  pronargs as argument_count,
  proargtypes::regtype[] as argument_types
FROM pg_proc 
...
-- Create a single, clean get_user_tasks function
...
SELECT 
  'Final get_user_tasks function verification:' as info,
  proname as function_name,
  pronargs as argument_count,
  proargnames as argument_names,
  proargtypes::regtype[] as argument_types
...
SELECT 
  'Test get_user_tasks function:' as info,
  task_number,
  variant_name,
  question_text,
  actions,
