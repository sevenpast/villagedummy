-- =====
-- CORRECT get_user_tasks FUNCTION - MATCH ACTUAL DATABASE TYPES
-- =====

-- First, drop the existing function
DROP FUNCTION IF EXISTS public.get_user_tasks(UUID, INTEGER);

-- Now create the function with ACTUAL database datatypes
CREATE OR REPLACE FUNCTION public.get_user_tasks(user_uuid UUID, module_num INTEGER DEFAULT 1)
RETURNS TABLE (
  task_id UUID,
  task_number INTEGER,
  title TEXT,  -- tasks.title is TEXT
  priority INTEGER,
  display_order INTEGER,
  variant_id UUID,
  variant_name CHARACTER VARYING,  -- task_variants.variant_name is CHARACTER VARYING
  intro_text TEXT,  -- task_variants.intro_text is TEXT
  info_box_content TEXT,  -- task_variants.info_box_content is TEXT
  question_text CHARACTER VARYING,  -- task_variants.question_text is CHARACTER VARYING
  actions JSONB,
  modal_title CHARACTER VARYING,  -- task_variants.modal_title is CHARACTER VARYING
  modal_content TEXT,  -- task_variants.modal_content is TEXT
  modal_has_reminder BOOLEAN,
  modal_default_reminder_days INTEGER,
  official_link_url TEXT,  -- task_variants.official_link_url is TEXT
  official_link_label CHARACTER VARYING,  -- task_variants.official_link_label is CHARACTER VARYING
  checklist_items JSONB,
  user_status TEXT,  -- user_task_progress.status is TEXT
