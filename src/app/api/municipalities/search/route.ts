...
// Swiss Federal Statistical Office API endpoint
const BFS_API_BASE = 'https://www.bfs.admin.ch/bfsapi/services/v1/statistics/'
...
export async function GET(request: NextRequest) {
  try {
    ...
    let municipalities: MunicipalityData[] = []

    // 1. Try OpenPLZ API (most comprehensive)
    try {
      const openPLZData = await searchOpenPLZAPI(query, limit)
      municipalities = [...municipalities, ...openPLZData]
    } catch (error) {
      console.log('OpenPLZ API not available, trying Swiss Post API')
    }

    // 2. Try Swiss Post API (if OpenPLZ fails)
    if (municipalities.length === 0) {
      try {
        const swissPostData = await searchSwissPostAPI(query, limit)
        municipalities = [...municipalities, ...swissPostData]
      } catch (error) {
        console.log('Swiss Post API not available, using fallback')
      }
    }

    // 3. Fallback to comprehensive local data
    if (municipalities.length === 0) {
      municipalities = searchLocalMunicipalities(query, limit)
    }

    // Remove duplicates and sort
    const uniqueMunicipalities = removeDuplicates(municipalities)
      .sort((a, b) => a.municipalityName.localeCompare(b.municipalityName))
      .slice(0, limit)

    return NextResponse.json({ 
      municipalities: uniqueMunicipalities,
      total: uniqueMunicipalities.length,
      query: query
    })
...
async function searchOpenPLZAPI(query: string, limit: number): Promise<MunicipalityData[]> {
  try {
    // OpenPLZ API - Comprehensive Swiss municipalities data
    const response = await fetch(`https://openplzapi.org/de/switzerland/Places?name=${encodeURIComponent(query)}&limit=${limit}`)
    
    if (!response.ok) {
      throw new Error('OpenPLZ API not available')
    }
    ...
    console.log('OpenPLZ API error:', error)
    throw error
  }
}

async function searchSwissPostAPI(query: string, limit: number): Promise<MunicipalityData[]> {
  // Swiss Post API endpoint (if available)
  const response = await fetch(`https://swisspost.opendatasoft.com/api/records/1.0/search/?dataset=plz_verzeichnis_v2&q=${encodeURIComponent(query)}&rows=${limit}`)
  
  if (!response.ok) {
    throw new Error('Swiss Post API not available')
  }
  ...
  return data.records?.map((record: any) => ({
    ...
    municipalityName: record.fields?.ortbez18 || '',
    cantonCode: record.fields?.kanton || '',
    cantonName: record.fields?.kantonsname || ''
  })) || []
}

function searchLocalMunicipalities(query: string, limit: number): MunicipalityData[] {
  const lowercaseQuery = query.toLowerCase()
  
  // Comprehensive Swiss municipalities data
  const allMunicipalities: MunicipalityData[] = [
    // Zurich (ZH) - Major cities
